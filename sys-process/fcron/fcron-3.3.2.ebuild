...
SLOT="0"
KEYWORDS="*"
IUSE="audit debug pam selinux l10n_fr doc +mta +system-crontab readline"
...
src_compile() {
	emake DESTDIR="${D}" \
		fcron fcrontab fcrondyn convert-fcrontab files/fcron.conf initscripts

	if use doc; then
		emake DESTDIR="${D}" documentation
		# Gentoo bug #216460
		sed \
