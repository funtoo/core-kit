...
### NOTE!!: 6.5.0 has ABI breakage in std::pair handling, the following patch is required to avoid breakage vs. previous 6.x later 7.x versions.
GCC_6_5_0_FIX_ABI_BREAKAGE_PATCH="gcc-6.5.0-Fix-std__pair-ABI-libstdc++-PR87822.patch"
...
src_prepare() {
	...
	if [ -n "${GCC_6_5_0_FIX_ABI_BREAKAGE_PATCH}" ] ; then
		ewarn "GCC 6.5.0 had ABI breakage of std::pair in libstdc++."
		einfo "Applying '${GCC_6_5_0_FIX_ABI_BREAKAGE_PATCH}'."
		eapply "${FILESDIR}/${GCC_6_5_0_FIX_ABI_BREAKAGE_PATCH}" || die
		einfo "Setting version to '6.5.1'."
		printf -- '6.5.1\n' > "${S}/gcc/BASE-VER" || die
		ewarn "If you have anything compiled with gcc-6.5.0, please recompile with 6.5.1!"
	fi
