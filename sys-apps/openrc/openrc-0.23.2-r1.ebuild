...
pkg_postinst() {
	...
	echo
	for r in sysinit boot shutdown default; do
	...
	# remove broken symlinks that are overlooked leftovers with openrc-0.18 to 0.23.2 update. This 3 services are no longer present in 0.23.2
	[ -h $EROOT/etc/runlevels/sysinit/tmpfiles.dev ] && rm $EROOT/etc/runlevels/sysinit/tmpfiles.dev
	[ -h $EROOT/etc/runlevels/boot/tmpfiles.setup ] && rm $EROOT/etc/runlevels/boot/tmpfiles.setup
	[ -h $EROOT/etc/runlevels/boot/swapfiles ] && rm $EROOT/etc/runlevels/boot/swapfiles
