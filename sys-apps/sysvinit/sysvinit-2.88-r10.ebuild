...
src_prepare() {
	...
	epatch "${FILESDIR}"/${P}-sysmacros.patch #fixes build with newer glibc
	epatch "${FILESDIR}"/${P}-ttydefaults.patch #fixes build on armv7+musl.
	...
	# FL-942, pidof binary moved to procps
	sed -i -r \
	    -e '/\/bin\/pidof/d' \
		-e '/^MAN8/s:\<pidof.8\>::g' \
	    src/Makefile || die
	...
	cd "${WORKDIR}"
	cp "${FILESDIR}"/inittab-2.88-r9 inittab || die "cp inittab"
	local insert=()
