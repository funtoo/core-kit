...
src_prepare() {
	...
	# FL-FL-5063
	eapply "${FILESDIR}"/2.26/glibc-2.26-makecontext.patch
...
glibc_headers_fix_cuda_float128() {
       # Fix CUDA issue with float128
       einfo "Fixing ${D}$(alt_headers)/bits/floatn.h for CUDA:"
       sed -e '/#if (defined __x86_64__/,/# define __HAVE_FLOAT128/ { s/(defined __x86_64__/!defined(__CUDACC__) \&\& &/ }' \
               -i "${D}$(alt_headers)/bits/floatn.h" || die
}
