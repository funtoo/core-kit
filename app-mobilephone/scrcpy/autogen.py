...
async def generate(hub, **pkginfo):
	...
	src_url = latest_release["tarball_url"]
	src_name = f"{github_repo}-{version}.tar.gz"
	src_artifact = hub.pkgtools.ebuild.Artifact(url=src_url, final_name=src_name)
	server_url = next(x for x in latest_release["assets"] if "server" in x["name"])["browser_download_url"]
	server_artifact = hub.pkgtools.ebuild.Artifact(url=server_url)
	ebuild = hub.pkgtools.ebuild.BreezyBuild(
		**pkginfo,
		version=version,
		github_user=github_user,
		github_repo=github_repo,
		artifacts=[src_artifact, server_artifact]
	)
	ebuild.push()
