...
SRC_URI="
https://www.kernel.org/pub/software/scm/git/git-2.27.0.tar.xz -> git-2.27.0.tar.xz
https://www.kernel.org/pub/software/scm/git/git-manpages-2.27.0.tar.xz -> git-manpages-2.27.0.tar.xz"
KEYWORDS="*"

LICENSE="GPL-2"
SLOT="0"
IUSE="+blksha1 +curl cgi doc emacs gnome-keyring +gpg highlight +iconv libressl +nls +pcre +pcre-jit perforce +perl +ppcsha1 tk +threads +webdav xinetd cvs subversion test"
...
src_unpack() {
	unpack git-2.27.0.tar.xz
	cd "${S}" || die
	unpack git-manpages-2.27.0.tar.xz
	if use doc ; then
