...
DEB_PV="5.10.13-${DEB_EXTRAVERSION}"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/5.10.13/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/5.10.13/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.13/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/5.10.13/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.13/nocerts.patch" ]; then
	    epatch "${FILESDIR}"/5.10.13/nocerts.patch || die
	else
	    epatch "${FILESDIR}"/latest/nocerts.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.13/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/5.10.13/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.13/fix-bluetooth-polling.patch" ]; then
	    epatch "${FILESDIR}"/5.10.13/fix-bluetooth-polling.patch || die
	else
	    epatch "${FILESDIR}"/latest/fix-bluetooth-polling.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.13/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/5.10.13/extra_cpu_optimizations.patch || die
	else
