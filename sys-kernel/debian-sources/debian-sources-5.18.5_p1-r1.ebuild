...
DEB_PV="5.18.5-${DEB_EXTRAVERSION}"
...
SRC_URI="https://deb.debian.org/debian/pool/main/l/linux/linux_5.18.5.orig.tar.xz -> linux_5.18.5.orig.tar.xz https://deb.debian.org/debian/pool/main/l/linux/linux_5.18.5-1.debian.tar.xz -> linux_5.18.5-1.debian.tar.xz"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/5.18.5/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/5.18.5/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/5.18.5/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/5.18.5/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/5.18.5/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/5.18.5/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/5.18.5/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/5.18.5/extra_cpu_optimizations.patch || die
	else
