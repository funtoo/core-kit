...
DEB_PV="5.10.26-${DEB_EXTRAVERSION}"
...
SRC_URI="http://http.debian.net/debian/pool/main/l/linux/linux_5.10.26.orig.tar.xz http://http.debian.net/debian/pool/main/l/linux/linux_5.10.26-1.debian.tar.xz"
S="$WORKDIR/linux-5.10.26"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/5.10.26/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/5.10.26/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.26/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/5.10.26/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.26/nocerts.patch" ]; then
	    epatch "${FILESDIR}"/5.10.26/nocerts.patch || die
	else
	    epatch "${FILESDIR}"/latest/nocerts.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.26/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/5.10.26/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.26/fix-bluetooth-polling.patch" ]; then
	    epatch "${FILESDIR}"/5.10.26/fix-bluetooth-polling.patch || die
	else
	    epatch "${FILESDIR}"/latest/fix-bluetooth-polling.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.26/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/5.10.26/extra_cpu_optimizations.patch || die
	else
