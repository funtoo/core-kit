...
SRC_URI="https://deb.debian.org/debian/pool/main/l/linux/linux_6.0.3.orig.tar.xz -> linux_6.0.3.orig.tar.xz https://deb.debian.org/debian/pool/main/l/linux/linux_6.0.3-1.debian.tar.xz -> linux_6.0.3-1.debian.tar.xz"
S="$WORKDIR/linux-6.0.3"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/6.0.3/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/6.0.3/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/6.0.3/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/6.0.3/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/6.0.3/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/6.0.3/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/6.0.3/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/6.0.3/extra_cpu_optimizations.patch || die
	else
