...
DEB_PV="5.10.113-${DEB_EXTRAVERSION}"
...
SRC_URI="https://security.debian.org/debian-security/pool/updates/main/l/linux/linux_5.10.113.orig.tar.xz https://security.debian.org/debian-security/pool/updates/main/l/linux/linux_5.10.113-1.debian.tar.xz"
S="$WORKDIR/linux-5.10.113"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/5.10.113/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/5.10.113/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.113/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/5.10.113/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.113/nocerts.patch" ]; then
	    epatch "${FILESDIR}"/5.10.113/nocerts.patch || die
	else
	    epatch "${FILESDIR}"/latest/nocerts.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.113/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/5.10.113/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.113/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/5.10.113/extra_cpu_optimizations.patch || die
	else
