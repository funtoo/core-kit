...
DEB_PV="5.10.84-${DEB_EXTRAVERSION}"
...
SRC_URI="https://deb.debian.org/debian/pool/main/l/linux/linux_5.10.84.orig.tar.xz https://deb.debian.org/debian/pool/main/l/linux/linux_5.10.84-1.debian.tar.xz"
S="$WORKDIR/linux-5.10.84"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/5.10.84/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/5.10.84/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.84/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/5.10.84/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.84/nocerts.patch" ]; then
	    epatch "${FILESDIR}"/5.10.84/nocerts.patch || die
	else
	    epatch "${FILESDIR}"/latest/nocerts.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.84/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/5.10.84/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/5.10.84/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/5.10.84/extra_cpu_optimizations.patch || die
	else
