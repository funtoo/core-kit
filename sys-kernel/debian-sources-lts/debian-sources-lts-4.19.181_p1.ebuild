...
DEB_PV="4.19.181-${DEB_EXTRAVERSION}"
...
SRC_URI="http://http.debian.net/debian/pool/main/l/linux/linux_4.19.181.orig.tar.xz http://http.debian.net/debian/pool/main/l/linux/linux_4.19.181-1.debian.tar.xz"
S="$WORKDIR/linux-4.19.181"
...
src_prepare() {
	...
	if [ -e "${FILESDIR}/4.19.181/xfs-libcrc32c-fix.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/xfs-libcrc32c-fix.patch || die
	else
	    epatch "${FILESDIR}"/latest/xfs-libcrc32c-fix.patch || die
	fi
	if [ -e "${FILESDIR}/4.19.181/mcelog.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/mcelog.patch || die
	else
	    epatch "${FILESDIR}"/latest/mcelog.patch || die
	fi
	if [ -e "${FILESDIR}/4.19.181/nocerts.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/nocerts.patch || die
	else
	    epatch "${FILESDIR}"/latest/nocerts.patch || die
	fi
	if [ -e "${FILESDIR}/4.19.181/ikconfig.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/ikconfig.patch || die
	else
	    epatch "${FILESDIR}"/latest/ikconfig.patch || die
	fi
	if [ -e "${FILESDIR}/4.19.181/fix-bluetooth-polling.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/fix-bluetooth-polling.patch || die
	else
	    epatch "${FILESDIR}"/latest/fix-bluetooth-polling.patch || die
	fi
	if [ -e "${FILESDIR}/4.19.181/amdgpu-picasso.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/amdgpu-picasso.patch || die
	else
	    epatch "${FILESDIR}"/latest/amdgpu-picasso.patch || die
	fi
	if [ -e "${FILESDIR}/4.19.181/extra_cpu_optimizations.patch" ]; then
	    epatch "${FILESDIR}"/4.19.181/extra_cpu_optimizations.patch || die
	else
