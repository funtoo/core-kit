...
async def generate(hub, **pkginfo):
	...
	file_matches = (
		name_pattern.match(x.get("href")) for x in files_soup.find_all("a", href=True)
	)
	target_version = next(x.group(1) for x in file_matches if x)
	source_artifact = hub.pkgtools.ebuild.Artifact(
		url=f"{base_url}/zsh-{target_version}.tar.xz"
	)
	doc_artifact = hub.pkgtools.ebuild.Artifact(
		url=f"{base_url}/zsh-{target_version}-doc.tar.xz"
	)
	ebuild = hub.pkgtools.ebuild.BreezyBuild(
		**pkginfo, version=target_version, artifacts=[source_artifact, doc_artifact]
	)
	ebuild.push()
