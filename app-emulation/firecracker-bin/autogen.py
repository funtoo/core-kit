...
def get_release(parsed_json):
	releases = filter(
		...
		and x["name"].startswith(GITHUB_RELEASE_NAME),
		parsed_json,
	)
...
def get_artifact(hub, artifact_name, firecracker_arch, dl_assets):
	dl_asset = list(
		filter(
			...
			and x["name"].endswith(firecracker_arch),
			dl_assets,
		)
	).pop()
...
def get_artifacts(hub, release, funtoo_arch):
	try:
		...
		raise hub.pkgtools.ebuild.BreezyError(
			f"Can't find firecracker arch for {funtoo_arch}"
		)
	return [
		get_artifact(hub, a, firecracker_arch, release["assets"]) for a in ARTIFACTS
	]
