# Copyright 1999-2019 Funtoo Solutions, Inc.
# Distributed under the terms of the GNU General Public License v2

# @ECLASS: gnome3.eclass
# @MAINTAINER:
# drobbins@funtoo.org
...
# Deprecated, pass extra arguments to gnome3_src_configure.
...
gnome3_src_unpack() {
	if has ${EAPI:-0} 4 5; then
		...
		die "gnome3_src_unpack is banned from eapi6"
	fi
}
...
gnome3_src_configure() {
	...
	if has ${EAPI:-0} 4 5; then
		addwrite "$(unset HOME; echo ~)/.gnome3"
	else
		addpredict "$(unset HOME; echo ~)/.gnome3"
	fi
